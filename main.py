...
def main():
    ...
    for json_file in json_files:
        ...
        if existing:
            print(f"Skipping {json_file.name}, already in database (ID: {existing.id}).")
            continue
        ...
        try:
            ...
            prompt_user = summarizer_prompt.user.format(word_count=50, text=transcription_text)
            ...
            print(f"Successfully saved {json_file.name} to database (ID: {transcription.id})")
